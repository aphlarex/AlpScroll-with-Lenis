...
function init_scroll(elm, ratio, fixed) {
...
function fixed_scroll() {
  // als.fixed_scroll = als.fixed_scroll + (als.scroll_y - als.fixed_scroll) * als.ratio;
  als.fixed_scroll = als.scroll_y;
  document.querySelector(als.wrapper).style.setProperty("--scroll-yy", als.fixed_scroll);
}

// parallax
function Als_parallax(elm, limit, friction_y, friction_x) {
  if (elm) {
    // push
    als.parallax.push({
      elm: elm,
...
function Als_parallax_run() {
  for (let i = 0; i < als.parallax_data.length; i++) {
    ...
    // stop-at-center
      ...
      if (als.parallax_data[i].center_y > 0) {
        center_y = als.parallax_data[i].center_y;
      } else {
        center_y = 0;
      }
    ...
    // friction data
    als.parallax_data[i].center_y_friction = als.parallax_data[i].center_y_friction + (center_y - als.parallax_data[i].center_y_friction) * als.parallax_data[i].friction_y;
    als.parallax_data[i].center_x_friction = als.parallax_data[i].center_x_friction + (center_x - als.parallax_data[i].center_x_friction) * als.parallax_data[i].friction_x;
    // set style
...
// sticky
function Als_sticky(elm, limit, friction_y, friction_x) {
  if (elm) {
    // push
    als.sticky.push({
      elm: elm,
...
function Als_sticky_run() {
  for (let i = 0; i < als.sticky_data.length; i++) {
    ...
    // friction data
    als.sticky_data[i].stick_y_friction = als.sticky_data[i].stick_y_friction + (als.sticky_data[i].stick_y - als.sticky_data[i].stick_y_friction) * als.sticky_data[i].friction_y;
    als.sticky_data[i].stick_x_friction = als.sticky_data[i].stick_x_friction + (als.sticky_data[i].stick_x - als.sticky_data[i].stick_x_friction) * als.sticky_data[i].friction_x;
    // set style
...
// 2022-03-23
// sticky child
function Als_sticky_child(elm, limit) {
  if (elm) {
    // push
...
// reach element
function Als_reach(elm, offset_y, offset_x) {
  if (elm) {
    // push
    als.reach.push({
      elm: elm,
...
// mouse
function Als_mouse(elm, limit, friction_y, friction_x) {
  if (elm) {
    // push
    als.mouse.push({
      elm: elm,
...
function Als_mouse_run() {
  for (let i = 0; i < als.mouse_data.length; i++) {
    ...
    als.mouse_data[i].mouse_x_friction = als.mouse_data[i].mouse_x_friction + (als.mouse_data[i].mouse_x - als.mouse_data[i].mouse_x_friction) * als.mouse_data[i].friction_x;
    als.mouse_data[i].mouse_y_friction = als.mouse_data[i].mouse_y_friction + (als.mouse_data[i].mouse_y - als.mouse_data[i].mouse_y_friction) * als.mouse_data[i].friction_y;
    // set style
