...
function Als_split_data(elms) {
  ...
  for (let i = 0; i < the_elms.length; i++) {
    ...
    als.split_height = the_elms[i].offsetParent.clientHeight;
...
function watch_size() {
  ...
  if (als.split == 1) {
    if (als.split_data_detail[0].elm.offsetParent.clientHeight !== als.scroll_height) {
      als.scroll_height = als.split_data_detail[0].elm.offsetParent.clientHeight;
      Als_split_push();
    }
  }
}
